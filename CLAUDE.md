...
- El trigger `crear_perfil_al_registrar` decide el tipo por
  `raw_app_meta_data->>'tipo'` **en el INSERT** de `auth.users`. Toda cuenta
  se crea con `auth.admin.createUser` (`app_metadata: { tipo: 'admin' }` para
  administradores; sin él, usuario). Está en
